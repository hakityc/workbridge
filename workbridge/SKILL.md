---
name: workbridge
description: 跨 TAPD 与 Discourse/WiseFlow 处理需求、任务、讨论、进展和研发交付；按需连接账号并恢复未完成工作。用于平台业务操作及明确的跨平台协作，不用于无关的一般编码或聊天。
---
...
按目标加载一个连接器：TAPD → `references/connectors/tapd.md`；Discourse/WiseFlow → `references/connectors/discourse.md`。不要一次加载所有工具 Schema 或所有平台指导。
