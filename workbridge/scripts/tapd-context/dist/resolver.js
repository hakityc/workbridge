...
import { cachedWorkItemFromInput, contextIdFor, findCachedWorkItemsById, makeGitBranchBinding, readCachedWorkItem, readContextStore, readGitBranchBinding, readProject, writeCachedWorkItem, writeGitBranchBinding, } from "./store.js";
function maybeProject(repoRoot) {
    try {
...
function mergeCache(item, cache) {
    if (!cache) {
        return item;
    }
    return {
...
function candidateFromItem(branch, item, source, confidence, reason, extras = {}) {
    return {
        provider: "tapd",
...
export function candidateFromExplicitInput(branch, item, source, method, baseBranch, sourceBranch, commit) {
    return candidateFromItem(branch, item, source, 100, source === "explicit-url"
        ? "本轮显式 TAPD URL。"
...
function candidateFromBinding(branch, binding) {
    const item = {
        source: "tapd",
    ...
    return candidateFromItem(branch, mergeCache(item, cache), "git-dir-binding", 90, "当前分支在 $GIT_DIR/tapd-context 中有本机绑定。", {
...
function candidateFromBranchName(repoRoot, branch, project) {
    ...
    if (parsed.entity_type) {
        ...
        return candidateFromItem(branch, mergeCache(item, cache), "branch-name", confidence, cache
    ...
    if (cached.length === 1) {
        return candidateFromItem(branch, itemFromCache(cached[0]), "local-cache", 75, "分支名简写匹配到唯一的本机 TAPD 缓存。", {
            git: project
                ? {
                    base_branch: project.base_branch,
                    source_branch: branch,
    ...
    return candidateFromItem(branch, item, "branch-name", 40, "分支名使用 tapd-<id> 简写，缺少类型信息；默认按 Story 低置信度恢复。", {
        git: project
            ? {
                base_branch: project.base_branch,
                source_branch: branch,
...
function candidateFromLegacy(repoRoot, branch) {
    const store = readContextStore(repoRoot);
    const context = store.branches[branch];
    if (!context) {
        return null;
    }
    return candidateFromItem(branch, context.work_item, "legacy-context", 60, "读取到旧版 .tapd/context.json 当前分支绑定，并迁移到新本机存储。", {
...
export function persistCandidate(repoRoot, candidate, method = candidate.binding?.method || "bind") {
    const git = candidate.git || {
        base_branch: candidate.branch,
        source_branch: candidate.branch,
        created_from_commit: getHeadCommit(repoRoot),
    };
    const binding = makeGitBranchBinding(candidate.branch, candidate.workItem, candidate.source, method, git.base_branch, git.source_branch, git.created_from_commit, candidate.created_at);
    writeGitBranchBinding(repoRoot, binding);
    writeCachedWorkItem(cachedWorkItemFromInput(candidate.workItem, candidate.source));
}
export function resolveCurrent(repoRoot) {
