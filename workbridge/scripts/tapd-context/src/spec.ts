...
interface FlowAcceptance {
  id: string;
  priority: "must" | "should";
  statement: string;
}

interface FlowSpecManifest {
  version: 1;
  spec_id: string;
  title: string;
  source: {
  ...
  review?: {
    status: "draft" | "reviewing" | "approved" | "changes-required";
    reviewed_ref?: string;
    decided_at?: string;
  };
}

interface SpecInitInput {
  specId: string;
  title: string;
  workspaceId: string;
  document: string;
...
export interface SpecValidationResult {
  manifest: FlowSpecManifest;
  exactRef: string;
  providerSupported: boolean;
  localReviewGateValid: boolean;
}

export interface FlowRequirementDraft {
  title: string;
  idempotency_key: string;
  managed_description: string;
}

function objectValue(value: unknown, path: string, issues: string[]): Record<string, unknown> {
...
function repoUrlValue(value: unknown, path: string, issues: string[]): string {
...
function stringArray(value: unknown, path: string, issues: string[], minItems = 0): string[] {
...
function safeRepoPath(value: string, path: string, issues: string[]): string {
  if (!value) {
    return value;
  }
...
function parseManifest(value: unknown, repoRoot: string): SpecValidationResult {
  const issues: string[] = [];
  const root = objectValue(value, "manifest", issues);
  rejectUnknownKeys(
    root,
    ["version", "spec_id", "title", "source", "scope", "acceptance", "publication", "review"],
    "manifest",
    issues,
  );
  if (root.version !== 1) {
  ...
  rejectUnknownKeys(
    source,
    ["repo_url", "ref", "document", "prototype_paths", "decisions"],
    "source",
    issues,
  );
  ...
  const document = safeRepoPath(
    stringValue(source.document, "source.document", issues),
    "source.document",
    issues,
  );
  const prototypePaths = stringArray(
    source.prototype_paths,
    "source.prototype_paths",
    issues,
    1,
  ).map((path, index) =>
    safeRepoPath(path, `source.prototype_paths[${index}]`, issues),
  );
  const decisions =
    source.decisions === undefined
      ? undefined
      : safeRepoPath(
          stringValue(source.decisions, "source.decisions", issues),
          "source.decisions",
          issues,
        );
  ...
  const normalizedAcceptance: FlowAcceptance[] = [];
  if (Array.isArray(root.acceptance)) {
    root.acceptance.forEach((item, index) => {
      ...
      if (!(["must", "should"] as unknown[]).includes(acceptance.priority)) {
  ...
  if (publication) {
    rejectUnknownKeys(
      publication,
      ["provider", "space_id", "entity_type", "external_id", "url", "published_ref"],
      "publication",
      issues,
    );
    publication.provider = stringValue(
      publication.provider,
      "publication.provider",
      issues,
    );
    publication.space_id = stringValue(
    ...
    const mappingFields = ["external_id", "url", "published_ref"] as const;
  ...
  const review =
    root.review === undefined ? undefined : objectValue(root.review, "review", issues);
  if (review) {
    rejectUnknownKeys(
      review,
      ["status", "reviewed_ref", "decided_at"],
      "review",
      issues,
    );
  }
  ...
  if (
    review &&
    !(["draft", "reviewing", "approved", "changes-required"] as unknown[]).includes(
      reviewStatus,
    )
  ) {
  ...
  if (reviewStatus === "approved") {
    ...
    reviewedExactRef = exactFullCommitValue(
      repoRoot,
      reviewedRef,
      "review.reviewed_ref",
      issues,
  ...
  const manifest = root as unknown as FlowSpecManifest;
  return {
    manifest,
    exactRef,
    providerSupported:
      manifest.publication?.provider === "tapd" && Boolean(manifest.publication.space_id),
    localReviewGateValid:
...
export function readAndValidateFlowSpec(repoRoot: string): SpecValidationResult {
...
export function renderFlowRequirement(
  result: SpecValidationResult,
): FlowRequirementDraft {
...
function remoteUrl(repoRoot: string): string {
  let value = "";
  try {
    ...
    throw new CliError(
      ...
      "产品仓库缺少 origin remote，无法生成可追溯的规格来源。",
    );
  }
  const issues: string[] = [];
  ...
  if (issues.length > 0) {
    throw new CliError(
      "UNSAFE_GIT_REMOTE_URL",
      "origin remote 不能安全写入共享规格清单。",
      { issues },
    );
  }
  return safeValue;
...
function flowOutputPath(repoRoot: string): string {
...
function writeManifestAtomically(repoRoot: string, manifest: FlowSpecManifest): void {
...
function assertSourcePathsCommitted(repoRoot: string, paths: string[]): void {
  const output = runGit(["status", "--porcelain=v1", "--", ...paths], repoRoot);
  if (output) {
    throw new CliError(
      "SPEC_SOURCE_NOT_COMMITTED",
      "产品文档或原型存在未提交修改；请先提交，再重新生成规格清单。",
      { entries: output.split("\n").filter(Boolean) },
    );
  }
}

export function initializeFlowSpec(repoRoot: string, input: SpecInitInput): SpecValidationResult {
  const path = join(repoRoot, FLOW_SPEC_FILE);
  if (existsSync(path) && !input.force) {
    throw new CliError(
      "SPEC_ALREADY_INITIALIZED",
      ".flow/spec.json 已存在；确认覆盖时使用 --force。",
    );
  }
  const ref = getHeadCommit(repoRoot);
  const manifest: FlowSpecManifest = {
    version: 1,
