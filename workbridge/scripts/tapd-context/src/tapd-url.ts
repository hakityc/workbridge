import { CliError, normalizeWorkItem, WorkItemInput } from "./schema.js";

const STANDARD_DETAIL_PATH =
  /^\/tapd_fe\/([^/]+)\/(story|task|bug)\/detail\/([^/?#]+)\/?$/i;
const PRONG_VIEW_PATH =
  /^\/([^/]+)\/prong\/(stories|tasks)\/view\/([^/?#]+)\/?$/i;
const BUGTRACE_VIEW_PATH =
  /^\/([^/]+)\/bugtrace\/bugs\/view\/([^/?#]+)\/?$/i;

export function parseTapdUrl(rawUrl: string): WorkItemInput {
  let url: URL;
  try {
  ...
  if (!identity) {
    throw new CliError(
      ...
      "仅支持标准 detail、Story/Task prong view 和 Bug bugtrace view 链接。",
      { url: rawUrl },
    );
  }
...
export function mergeUrlIdentity(input: WorkItemInput): WorkItemInput {
  if (!input.url) {
    return input;
  }
...
export function parseInput(rawInput: string): WorkItemInput {
