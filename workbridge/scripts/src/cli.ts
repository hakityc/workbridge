...
function connectionPath(name: string): string {
  return join(home(), "connections", identifier(name) + ".json");
}
function load(name: string): ConnectionProfile {
  const p = readPrivate<ConnectionProfile>(connectionPath(name));
  requireValue(
    p.version === 1 &&
      p.name === name &&
      ["codex", "claude", "cursor"].includes(p.host) &&
      ["stdio", "streamable-http"].includes(p.transport),
    "INVALID_CONNECTION",
    "连接配置无效。",
  );
  identifier(p.server);
  ...
  if (p.transport === "streamable-http") {
    ...
    safeUrl(p.url, true);
    if (p.auth === "bearer-env") requireValue(p.host === "codex" && typeof p.bearerEnv === "string" && /^[A-Z][A-Z0-9_]*$/.test(p.bearerEnv), "UNSUPPORTED_AUTH", "远程凭据引用无效。");
  }
  return p;
}
function output(value: unknown): void {
  process.stdout.write(JSON.stringify(value, null, 2) + "\n");
  const status = value as { ok?: boolean; probe?: { ok: boolean } };
  if (status?.ok === false || status?.probe?.ok === false) process.exitCode = 1;
}
function available(command: string, args = ["--version"]): void {
  const r = spawnSync(command, args, { stdio: "ignore", timeout: 10000 });
  requireValue(
    !r.error && r.status === 0,
    "DEPENDENCY_MISSING",
    `需要可运行的 ${command}；安装后重新执行原连接命令。`,
  );
}
async function about(site: string): Promise<void> {
  try {
    ...
    const data = (await response.json()) as { about?: unknown };
    requireValue(
      data.about,
      "NOT_DISCOURSE",
      "站点没有返回 Discourse about 信息。",
    );
  } catch (e) {
...
async function generateProfile(site: string, path: string): Promise<void> {
  requireValue(
  ...
  try {
    await new Promise<void>((resolvePromise, reject) => {
      ...
      const child = spawn(
        c.command,
        [...c.args, "generate-user-api-key", "--site", site, "--save-to", path],
        { stdio: "inherit" },
      );
...
function validateDiscourseProfile(path: string, site: string): void {
  ...
  requireValue(
    value.auth_pairs?.some(
      (p) =>
        p.site?.replace(/\/$/, "") === site && (p.user_api_key || p.api_key),
    ),
    "AUTH_REQUIRED",
    "profile 没有该站点的凭据。",
  );
}
function restoreSwitch(old: ConnectionProfile, next: ConnectionProfile): void {
  ...
  if (actual !== undefined) {
    requireValue(
      matchesRegistration(actual, next, cli),
      "HOST_CONFLICT",
      "宿主配置已被其他操作修改，不能自动恢复。",
    );
...
async function main(): Promise<void> {
  ...
  if (cmd === "context") {
    if (sub === "current") output(current());
      ...
      if (existsSync(connectionPath(r.connection)))
        requireValue(
          load(r.connection).connector === r.provider,
          "INVALID_RESOURCE",
          "资源与连接器不匹配。",
        );
      else connector(r.provider);
      if (sub === "start") {
        requireValue(
          v.base && v.branch,
          "INVALID_ARGUMENT",
          "需要已确认的 --base 与 --branch。",
        );
        output(await start(r, v.base, v.branch));
      } else output(bind(r, v.replace));
    } else throw new BridgeError("INVALID_ARGUMENT", "未知 context 命令。");
    return;
  }
  if (cmd === "run") {
    if (sub === "show") output(loadRun(name));
    else if (sub === "save") {
      requireValue(v.input, "INVALID_ARGUMENT", "需要 --input。");
      if (v["no-persist"]) output({ ok: true, persisted: false });
      else output(saveRun(JSON.parse(readFileSync(v.input, "utf8"))));
    } else throw new BridgeError("INVALID_ARGUMENT", "未知 run 命令。");
    return;
  }
  if (cmd === "mcp-launch") {
    const p = load(sub);
    ...
    await new Promise<void>((done, reject) => {
      child.on("error", () =>
        reject(new BridgeError("SERVER_START_FAILED", "MCP 无法启动。")),
      );
      child.on("exit", (code) => {
  ...
  if (cmd === "doctor") {
    requireValue(
      v.host && ["codex", "claude", "cursor"].includes(v.host) && v.connection,
      "INVALID_ARGUMENT",
      "需要 --host 和 --connection。",
    );
    if (!existsSync(connectionPath(v.connection))) {
      const existing = inspect(v.host as Host, v.connection);
      output({
        ok: false,
  ...
  await locked(async () => {
    if (cmd === "disconnect") {
      const p = load(sub);
      await authorize(
        `从 ${p.host} 移除 WorkBridge 服务 ${p.server}？不会撤销远端 key。`,
        p.name,
        !!v.yes,
      );
      removeHost(p);
    ...
    if (cmd === "connection" && sub === "configure") {
      const old = load(name);
      requireValue(
        v.transport === "stdio" || (v.transport === "streamable-http" && v.url),
        "INVALID_ARGUMENT",
        "需要 --transport stdio，或 --transport streamable-http --url。",
      );
      const auth = v.auth || "oauth";
      requireValue(
        ["oauth", "none", "bearer-env"].includes(auth),
        "UNSUPPORTED_AUTH",
        "不支持的认证方式。",
      );
      if (auth === "bearer-env")
        requireValue(
          v["bearer-env"] && /^[A-Z][A-Z0-9_]*$/.test(v["bearer-env"]),
          "INVALID_ARGUMENT",
          "需要环境变量名称，不能提供 token 值。",
        );
      const p: ConnectionProfile =
        v.transport === "stdio"
          ...
          : {
              ...
              auth: auth as ConnectionProfile["auth"],
      ...
      if (p.transport === "stdio") {
        ...
        requireValue(
          Number(process.versions.node.split(".")[0]) >= c.node,
          "NODE_VERSION",
          `本地连接需要 Node ${c.node}+。`,
        );
        available(c.command);
      ...
      registration(p, cli);
      await authorize(
        `将 ${name} 切换为 ${p.url || "本地 stdio"}，保留原资源关联？`,
        name,
        !!v.yes,
      );
      const check = await verify(p);
      requireValue(
        check.ok || check.code === "HOST_AUTH_REQUIRED",
        check.code || "PROBE_FAILED",
        "远程探测失败，原连接未修改。",
      );
      ...
      requireValue(
        !existsSync(journal),
        "RECOVERY_REQUIRED",
        "有未完成的切换，请先运行 connection recover。",
      );
      ...
      try {
        ...
        try {
          ...
          throw new BridgeError(
            "RECOVERY_REQUIRED",
            "切换未完成；已保留恢复记录。请运行 connection recover <name>，不重复连接或写业务数据。",
          );
        }
        throw e;
      }
    ...
    requireValue(
      cmd === "connect" && ["tapd", "discourse"].includes(sub),
      "INVALID_ARGUMENT",
      "未知命令或连接器。",
    );
    requireValue(
    ...
    const connection = identifier(v.connection),
      c = connector(sub),
      host = v.host as Host;
    requireValue(
      Number(process.versions.node.split(".")[0]) >= c.node,
      "NODE_VERSION",
      `${sub} 本地 MCP 需要 Node.js ${c.node}+；请升级后重试，尚未请求凭据。`,
    );
    ...
    if (host !== "cursor") available(host);
    const path = connectionPath(connection);
    let prior: ConnectionProfile | undefined;
    if (existsSync(path)) {
      const old = load(connection);
      requireValue(
        old.host === host && old.connector === sub,
        "CONNECTION_CONFLICT",
        "连接名称已用于不同平台或宿主。",
      );
      requireValue(
        !existsSync(join(home(), "transactions", connection + ".json")),
        "RECOVERY_REQUIRED",
        "请先运行 connection recover，恢复中断的切换。",
      );
      const actual = inspect(host, old.server);
      if (actual === undefined || !old.hostFingerprint) {
        await authorize(
          `恢复未完成的 ${old.server} 宿主注册？`,
          connection,
          !!v.yes,
        );
        if (actual === undefined) {
          ...
          requireValue(
            matchesRegistration(actual, old, cli),
            "HOST_CONFLICT",
            "配置不是计划中的 WorkBridge 注册，未接管。",
          );
        ...
        requireValue(
          fingerprint(actual) === old.hostFingerprint,
          "HOST_CONFLICT",
          "宿主配置已被外部修改；未覆盖。",
        );
      prior = old;
      if (v["allow-writes"] && !old.allowWrites && !v.replace) {
        requireValue(
          old.connector === "discourse" && old.transport === "stdio",
          "UNSUPPORTED_OPERATION",
          "写入模式由远程服务管理。",
        );
        await authorize(
          "启用该 Discourse 连接的写工具？业务发帖仍需明确授权。",
          connection,
          !!v.yes,
        );
        const next = {
          ...old,
          allowWrites: true,
          state: "pending-host-reload" as const,
        };
    ...
    await authorize(
      `将为 ${host} 新增 ${server}，凭据保存在仓库外的受限文件。继续？`,
      connection,
      !!v.yes,
    );
    ...
    try {
      ...
      if (sub === "tapd") {
        process.stdout.write(
          "请从 https://www.tapd.cn/personal_settings/index?tab=personal_token 获取 token。不要粘贴到聊天。\n",
        );
        const token = String(
          await interact({
            version: 1,
        ...
        requireValue(
          check.ok,
          check.code || "AUTH_FAILED",
          "TAPD 只读认证验证未通过；未保存凭据或修改宿主。",
        );
      ...
      const p: ConnectionProfile = {
        ...
        credentialRef,
        allowWrites:
          sub === "tapd" || !!v["allow-writes"] || !!prior?.allowWrites,
        state: "pending-host-reload",
      };
      if (sub === "discourse") {
        const check = await verify(p);
...
main().catch((e) => {
  ...
  const stream =
    process.argv[2] === "mcp-launch" ? process.stderr : process.stdout;
