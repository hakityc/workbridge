...
const directory = fileURLToPath(
  new URL("../../references/connectors/", import.meta.url),
);
export function connectors(root = directory): Connector[] {
  return readdirSync(root)
    .filter((x) => x.endsWith(".json"))
    .map((x) => {
      const c = JSON.parse(readFileSync(join(root, x), "utf8")) as Connector;
      requireValue(
        c.version === 1 && c.id && c.probe?.tool && c.capabilities,
        "INVALID_CONNECTOR",
        "连接器声明无效。",
      );
      identifier(c.id);
...
export function localLaunch(
  profile: ConnectionProfile,
  c = connector(profile.connector),
  ...
  if (profile.connector === "tapd") {
    const value =
      ...
      (profile.credentialRef
        ? readPrivate<Record<string, unknown>>(profile.credentialRef)
        : {});
    requireValue(
    ...
    requireValue(
      profile.site && profile.credentialRef,
      "AUTH_REQUIRED",
      "需要站点和 Discourse profile。",
    );
    ...
    if (profile.allowWrites) args.push("--allow_writes");
  }
